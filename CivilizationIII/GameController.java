...
public class GameController {
    ...
    public static void setLastClicked(TerrainTileFX last) {
        ...
        if (tile.isEmpty()) {                           //empty
            if (nearSettlement(last)) {
                ...
                //This will probably not be used but I'll
                //place it here anyways
                state = GameState.NEUTRAL;
            }
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (!(end.isEmpty() && GridFX.adjacent(end, start)
            ...
            alert.setTitle("Move Action Failure");
            alert.setHeaderText("The move action could not be performed.");
            alert.showAndWait();
            return false;
        }
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            Alert alert = new Alert(AlertType.INFORMATION);
            alert.setTitle("Attack Action Failure");
            alert.setHeaderText("The attack action cannot be performed");
            alert.showAndWait();
            state = GameState.NEUTRAL;
            return;
        }
